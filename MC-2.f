c
       ...
       parameter(L=30)
       integer MCTOT
       integer S(1:L,1:L),W(1:2)
       integer PBC(0:L+1)
       double precision x,genrand_real2,mag,magne
       double precision energia,pr_1,TEMP,delta
       double precision rnd(1:2),enebis,sumaveins
       integer candidat_i,candidat_j,f,g,DE
       
c Parametres d'entrada
       SEED=23456
       TEMP=2.0d0
       MCTOT=10000
       Nom='SIM-L-030-TEMP-2000-MCTOT-10K.dat'
       N=L*L
       do DE=4,8,4
        W(DE/4)=dexp(-dble(DE)/TEMP)
       enddo
       open(1,file=Nom)
...
c Cridar el generador
       call init_genrand(SEED)

c Generació matriu espins
       pr_1=0.5d0
       do i=1,L
        do j=1,L
         x=genrand_real2()
        ...
        enddo
       enddo
c       call writeconf(S,L)
       mag=magne(S,L)
       call energ(S,L,PBC,energia)
       enebis=energia
       write(1,*) IMC,mag,energia,enebis

C Implementació Metropolis
       do IMC=1,MCTOT
        do IPAS=1,N
         i=int(genrand_real2()*L)+1
         j=int(genrand_real2()*L)+1
         sumaveins=S(PBC(i-1),j)+S(PBC(i+1),j)+
     +   S(i,PBC(j-1))+S(i,PBC(j+1))
         DE=2.d0*S(i,j)*sumaveins
         if (DE.le.(0.d0)) then 
          S(i,j)=-S(i,j)
          energia=energia+DE
         else
          delta=genrand_real2()
          if (delta.lt.(W(DE/4))) then 
           S(i,j)=-S(i,j)
           energia=energia+DE
          endif
         endif
...
c *WRITECONFIG*
c *************
       subroutine writeconf(S,L)
       integer L
       integer S(1:L,1:L)
       open(1,file='P1-configuration.conf')
       do i=1,L
        do j=1,L
         if (S(i,j).gt.0.d0) then
          write(1,*) i,j
         endif
        enddo
       enddo
...
c ***********
       ...
       integer L,i,j
       integer S(1:L,1:L)
       double precision imantacio
       imantacio=0.d0
       do i=1,L
        do j=1,L
         imantacio=imantacio+dble(S(i,j))
        enddo
       enddo
       magne=imantacio
       return
...
c *Energia*
c *********
      subroutine energ(S,L,PBC,energia)
      integer L,i,j
      integer S(1:L,1:L)
      integer PBC(0:L+1)
      double precision energia,hamilton
      energia=0.d0
      do i=1,L
       do j=1,L
         hamilton=-S(i,j)*S(PBC(i+1),j)-S(i,j)*S(i,PBC(j+1))
         energia=energia+hamilton
       enddo 
      enddo
      end subroutine

c **********************
c *Diferencia d'energia*
c **********************      
      subroutine diferencia(S,i,j,L,PBC,DE)
      integer L,i,j
      integer S(1:L,1:L)
      integer PBC(0:L+1)
      double precision sumaveins,DE
      sumaveins=S(PBC(i-1),j)+S(PBC(i+1),j)+S(i,PBC(j-1))+S(i,PBC(j+1))
      DE=2.d0*S(i,j)*sumaveins
      end subroutine

c ****************************
c *Nombres aleatoris discrets*
c ****************************
      subroutine candidat(x,L,candidati,candidatj)
      double precision x(1:2)
      integer candidati,candidatj
      candidati=int(x(1)*L)+1
      candidatj=int(x(2)*L)+1
      end subroutine
